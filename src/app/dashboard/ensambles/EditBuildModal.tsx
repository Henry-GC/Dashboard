  ...
  const formik = useFormik({
    ...
    onSubmit: async (values) => {
      ...
      try {
        ...
        await axios.put(`/adm/builds/update/${build.id}`, buildData);
        
        const updatedBuild = { ...build, ...buildData };
        onSave(updatedBuild);
        
        toast.success("Ensamble actualizado correctamente", {
          style: { background: '#22c55e', color: '#fff' },
        });
        
        onClose();
      } catch (err) {
        console.error("Error al actualizar ensamble:", err);
        toast.error("Error al actualizar el ensamble");
      }
    },
  });

  // Initialize components when build changes
  useEffect(() => {
    if (build?.components) {
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 overflow-y-auto">
      <div className="bg-white dark:bg-zinc-900 p-6 rounded shadow-xl w-full max-w-2xl max-h-[90vh] overflow-y-auto">
        <h2 className="text-xl font-semibold mb-4">Editar ensamble</h2>
